...
- **Dashboard:** four stacked rows — a 4-up stat row; two charts side by side from `xl` (attendance line + fee bar); class performance beside a recent-activity list; then upcoming exams, pending fees and the school calendar at a third each. The **stat cards are filled** in their metric's tone (see Components) — colour marks meaning (success = healthy, error = overdue or pending debt, warning = due soon, primary = counts and dates), never decoration — while every other card on the page stays a neutral surface with a single tinted icon chip. A KPI shows a real number or an honest empty state — never a dash placeholder. The **School calendar** card repeats that shape in miniature: the heading, then the month with its chevrons, the grid, then an `UPCOMING` list. A dated day carries a **dot** under its number, never a filled cell — an exam season marks most of the month, and filling each one turns the grid into a wall of indigo chips where the dates stop reading as dates. Today stays the one filled circle in the grid, and its marker inverts to white.
