...
- Phone fields use `PhoneInput` (`components/ui/PhoneInput.tsx`) — its prefix is the country flag and calling code, and its value is E.164, so it is bound through `Controller` (never `register()`), with `phoneRules` / `optionalPhoneRules` from `lib/validation.ts`; never a bare `Input type="tel"`. The field is hand-rolled, with no phone library: the dialling data lives in `lib/countries.ts` (generated from `libphonenumber-js` metadata once, then committed) and the flags are the SVGs in `public/flags/`, so nothing is fetched at runtime
