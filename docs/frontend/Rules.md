...
- Creates and edits open in the `Sheet` rather than navigating away; row actions live in a trailing ellipsis `Dropdown`, and every destructive one goes through `ConfirmDialog` first. Writes invalidate the feature's query key instead of patching the cache by hand
